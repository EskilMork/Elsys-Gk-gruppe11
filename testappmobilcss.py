...
# Detect Raspberry Pi-like environment and set Pi-specific SDL driver only there.
...
try:
    ...
    if os.path.exists("/etc/rpi-issue") or os.path.exists("/boot/config.txt"):
        is_raspberry_pi = True
except Exception:
    is_raspberry_pi = False
...
# Only attempt to import gpiozero on Raspberry Pi environments. On non-Pi systems
# provide a harmless stub so code referencing Button instances still works.
if not DISABLE_GPIO and is_raspberry_pi:
    try:
    ...
    class Button:  # stub for non-Pi environments
        def __init__(self, *args, **kwargs):
            self.when_pressed = None
            self.when_released = None
            self.is_pressed = False
...
def index(request: Request):
    """Return index.html for desktop and indextestmobilcss.html for mobile user agents.

    This is a lightweight server-side mobile detection using the User-Agent header.
    It serves the mobile-specific HTML which in turn loads `/static/mobil.css`.
    """
    ua = (request.headers.get("user-agent") or "").lower()
    mobile_indicators = ["mobile", "android", "iphone", "ipad", "ipod", "windows phone"]
    try:
        if any(tok in ua for tok in mobile_indicators):
            return FileResponse(os.path.join(STATIC_DIR, "indextestmobilcss.html"))
    except Exception:
        # fallback to desktop index if anything unexpected happens while checking UA
        pass
...
@app.get("/favicon.ico")
def favicon():
    """Return a small response for favicon requests to avoid 404 noise.

    If a real `static/favicon.ico` exists it will be served, otherwise return
    a 204 No Content so the browser console isn't cluttered.
    """
...
def update_caption(caption: Caption):
    ...
    print("[update_caption] incoming payload:", caption.dict())
    ...
    print("[update_caption] returning shared_data id:", shared_data.get('id'))
    return {"message": "Ny caption lagret!", "data": shared_data}


@app.get('/debug_status')
def debug_status():
    """Return the current server shared_data for debugging in the browser."""
    return {"shared_data": shared_data, "last_persisted": last_persisted_poll}


@app.get('/create_poll/')
def create_poll_via_get(name: Optional[str] = None):
    """Create a new poll using query param `name`.

    This is a convenience endpoint for debugging browser reachability. It
    mirrors the 'new poll' flow used by the client when creating a poll.
    """
    if not name:
        raise HTTPException(status_code=400, detail="`name` query parameter required")
    new_id = uuid.uuid4().hex[:8]
    # Reuse the same logic as update_caption for creating a new poll
    shared_data['caption'] = name
    shared_data['id'] = new_id
    shared_data['image_path'] = None
    mark_image_dirty()
    ...
    save_poll(force=True)
    print(f"[create_poll_via_get] created poll id={new_id} name={name}")
    return {"message": "Ny caption lagret (via GET)!", "data": shared_data}


@app.get('/echo_headers')
def echo_headers(request: Request):
    """Return select request headers so the browser can confirm the request reached the API."""
    headers = {k: v for k, v in request.headers.items() if k.lower() in ('host', 'origin', 'referer', 'user-agent')}
    print("[echo_headers]", headers)
...
"""
Register the upload endpoint only if python-multipart is available. FastAPI
requires the 'python-multipart' package to be installed to accept form/file
uploads; if it's not installed, avoid registering the handler that depends on
UploadFile/File/Form (which would raise at import time) and instead register a
placeholder that returns a helpful error message.
"""

try:
    import multipart  # type: ignore - provided by the python-multipart package
    MULTIPART_AVAILABLE = True
except Exception:
    MULTIPART_AVAILABLE = False
    print("python-multipart not installed; '/upload_image/' endpoint will be disabled. Install with: pip install python-multipart")
...
if MULTIPART_AVAILABLE:
    ...
    @app.post("/upload_image/")
    async def upload_image_disabled():
        raise HTTPException(
            status_code=501,
            detail=(
                "File upload support is disabled because 'python-multipart' is not installed. "
